...
It's a good idea to define the \texttt{shorthand} field in the bib file for BIPs, RFCs and similar documents, \eg, \verb|shorthand = {BIP340}|. By the way, have you heard about x-only keys~\cite{add:bip-schnorr,C:NicRufSeu21}?
...
\paragraph{How to Fix PDF Metadata?}
PDF metadata such as bookmarks created from section headings,
  ...
  \begin{center}
    \begin{pchstack}[center, boxed]
      \begin{pcvstack}
        ...
        \pcvspace
        \procedure[headlinesep=1pt]{$\gametitle{\game{Coll}}{\algo{H}}$}{%
          \mathellipsis
        }
      \end{pcvstack}
      \pchspace[1em]
      \begin{pcvstack}
        \procedure{$\gametitle*{\game{EUF\mhyphen{}CMA}}{\Sigma}[\bdv]$}{%
          x \gets 5\\
          \gamechange{\pcassert \pcfalse}
        }
        \pcvspace
        \procedure[headlinesep=1pt]{$\oracle{Oracle}(\sk, \pk)$}{%
          y \sample \ZZ_p
        }
      \end{pcvstack}
    \end{pchstack}
